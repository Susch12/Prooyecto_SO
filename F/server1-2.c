...
void *manejar_cliente(void *arg) {
    ...
    // Bucle para los 3 exámenes
    for (int examen = 0; examen < 3; examen++) {
        int materia;
        char menu[] = "Seleccione un examen:\n1. Matemáticas\n2. Español\n3. Inglés\n";
        send(client_sock, menu, strlen(menu) + 1, 0);

        // Recibir la selección del usuario
        read(client_sock, &materia, sizeof(materia));
        if (materia < 1 || materia > 3) {
            char error[] = "Selección inválida. Intente de nuevo.\n";
            send(client_sock, error, strlen(error) + 1, 0);
            examen--; // Reintentar el examen actual
            continue;
        }

        // Ajustar el índice de la materia
        int materia_idx = materia - 1;

        // Iniciar el examen seleccionado
        int correctas = 0;
        ...
        for (int i = 0; i < 10; i++) {
            ...
            send(client_sock, preguntas[materia_idx][pregunta_id].pregunta, 
                 strlen(preguntas[materia_idx][pregunta_id].pregunta) + 1, 0);

            for (int j = 0; j < 3; j++) {
                send(client_sock, preguntas[materia_idx][pregunta_id].opciones[j], 
                     strlen(preguntas[materia_idx][pregunta_id].opciones[j]) + 1, 0);
            }
            ...
            // Verificar respuesta
            if (respuesta == preguntas[materia_idx][pregunta_id].respuesta_correcta) {
                correctas++;
            }
        }
        ...
        pthread_mutex_lock(&lock);
        usuarios[user_index].calificaciones[materia_idx] = correctas;
        pthread_mutex_unlock(&lock);
    }
